...
happyExpList = Happy_Data_Array.listArray (0,45) ([21440,1,4,0,0,0,0,0,0,8256,20224,7,0,33,0,144,128,0,21440,257,0,61440,84,0,0,128,0,21440,1,0,0
	])

{-# NOINLINE happyExpListPerState #-}
happyExpListPerState st =
    token_strs_expected
  where token_strs = ["error","%dummy","%start_parseCalc","JsonG","Elem","Obj","str","number","true","false","','","':'","'{'","'}'","'['","']'","null","%eof"]
        bit_start = st * 18
        bit_end = (st + 1) * 18
...
happyReduction_2 (HappyTerminal (Chain happy_var_1))
	 =  HappyAbsSyn4
		 (GString happy_var_1
	)
...
happyReduction_3 (HappyTerminal (Literal happy_var_1))
	 =  HappyAbsSyn4
		 (GNumber (read happy_var_1::Double)
	)
...
happyReduction_12 (HappyAbsSyn4  happy_var_3)
	_
	(HappyTerminal (Chain happy_var_1))
	 =  HappyAbsSyn6
		 (Data.Map.fromList [(happy_var_1,happy_var_3)]
	)
...
happyReduction_13 ((HappyAbsSyn4  happy_var_5) `HappyStk`
	_ `HappyStk`
	(HappyTerminal (Chain happy_var_3)) `HappyStk`
	_ `HappyStk`
	(HappyAbsSyn6  happy_var_1) `HappyStk`
	happyRest)
	 = HappyAbsSyn6
		 (Data.Map.insert happy_var_3 happy_var_5 happy_var_1
	) `HappyStk` happyRest
...
happyNewToken action sts stk (tk:tks) =
	...
	case tk of {
	Chain happy_dollar_dollar -> cont 7;
	Literal happy_dollar_dollar -> cont 8;
	TrueB -> cont 9;
	FalseB -> cont 10;
	Comma -> cont 11;
	Colon -> cont 12;
